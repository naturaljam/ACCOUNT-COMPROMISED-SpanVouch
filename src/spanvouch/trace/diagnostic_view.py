...
def _sanitize_attributes(attributes: dict[str, JsonValue]) -> dict[str, JsonValue]:
    sanitized = sanitize_diagnostic_value(attributes)
    if isinstance(sanitized, dict):
        return sanitized
    return {"sanitization": SECRET_REDACTION}


def _sanitize_span(span: DiagnosticSpan) -> DiagnosticSpan:
    return DiagnosticSpan(
        span_id=cast(str, sanitize_diagnostic_value(span.span_id)),
        parent_span_id=_sanitize_optional_string(span.parent_span_id),
        name=cast(str, sanitize_diagnostic_value(span.name)),
        kind=span.kind,
        status=span.status,
