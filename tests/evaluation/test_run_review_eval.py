...
def test_hybrid_cli_requires_live_opt_in_before_credentials_or_provider(
    ...
    monkeypatch.setattr(
        "spanvouch.evaluation.run_review_eval.DeepSeekConfig.from_env", forbidden_config
    )
...
def test_hybrid_cli_runs_only_allowlisted_candidates_and_reports_semantic_metrics(
    ...
    monkeypatch.setattr(
        "spanvouch.evaluation.run_review_eval.DeepSeekConfig.from_env", lambda: object()
    )
    monkeypatch.setattr("spanvouch.evaluation.run_review_eval.DeepSeekProvider", make_provider)
